...
static void make_bl_default_point(struct bl_info *bl)
{
	...
	if (IS_ERR_OR_NULL(bl->bl_default_point)) {
		bl->bl_default_point = kcalloc(BL_POINT_IDX_MAX, sizeof(unsigned int), GFP_KERNEL);

		bl->bl_default_point[BL_POINT_OFF] = 0;
		bl->bl_default_point[BL_POINT_MIN] = 1;
		bl->bl_default_point[BL_POINT_DFT] = bl->bd->props.brightness;
		bl->bl_default_point[BL_POINT_MAX] = 255;
		bl->bl_default_point[BL_POINT_OUT] = bl->bd->props.max_brightness;

		for (i = 0; i <= bl->bl_default_point[BL_POINT_MAX]; i++) {
			if (bl->brightness_table[i]) {
				bl->bl_default_point[BL_POINT_MIN] = i;
				break;
			}
		}
	}
...
{
	int i, ret = 0;
	unsigned int bl_level[80] = {0, };
	unsigned int bl_value[80] = {0, };
	unsigned int max_bl_level;
	unsigned int max_bl_value;
	unsigned int off_bl_point = BL_POINT_IDX_MAX - 1;
	char *pos = NULL;
	...
	if (!max_bl_level) {
		...
		for (i = 0; i <= off_bl_point; i++)
			bl_level[i] = bl->bl_default_point[i];
					...
					size_t len, loff_t *ppos)
{
	...
	unsigned int command = 0, value = 0;
	int ret = 0;
	...
	if (clamp(command, 1U, 255U) != command) {
		dev_info(&ic->bd->dev, "%s: input is invalid, command: %02x\n", __func__, command);
		goto exit;
	}

	if (clamp(value, 0U, 255U) != value) {
...
int init_bl_curve_debugfs(struct backlight_device *bd, unsigned int *table, struct i2c_client **clients)
{
	...
	int ret = 0, i2c_count;
	char file_string[I2C_NAME_SIZE + 11];
	...
	for (i2c_count = 0; clients && clients[i2c_count]; i2c_count++) {
		...
		scnprintf(file_string, sizeof(file_string), "%s-%s", "ic_tuning", clients[i2c_count]->name);
		debugfs_create_file(file_string, S_IRUSR | S_IWUSR, debugfs_root, ic, &ic_tuning_fops);
	}
