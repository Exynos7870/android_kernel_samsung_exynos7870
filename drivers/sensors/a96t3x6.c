...
struct a96t3x6_data {
	...
	u8 checksum_l_bin;
	bool enabled;
	bool flip_mode;			/* flip_cover mode */
...
static int a96t3x6_input_open(struct input_dev *dev);
static void a96t3x6_input_close(struct input_dev *dev);
...
static int a96t3x6_i2c_write(struct i2c_client *client,
		u8 reg, u8 *val, unsigned int len)
{
	struct a96t3x6_data *data = i2c_get_clientdata(client);
...
sar_mode:
	while (retry > 0) {
		ret = a96t3x6_i2c_write(data->client, REG_SAR_MODE, &cmd, 1);
		if (ret < 0) {
...
static void a96t3x6_sar_sensing(struct a96t3x6_data *data, int on)
{
	...
	ret = a96t3x6_i2c_write(data->client, REG_SAR_SENSING, &cmd, 1);
	if (ret < 0)
		SENSOR_ERR("fail(%d)\n", ret);
}
...
static void a96t3x6_reset(struct a96t3x6_data *data)
{
	...
	if (data->sar_enable)
		a96t3x6_mode_enable(client, REG_SAR_ENABLE, CMD_ON);
		 ...
		 struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = a96t3x6_mode_enable(client, REG_SAR_ENABLE, cmd);
	if (ret < 0) {
		SENSOR_ERR("fail(%d)\n", ret);
		return count;
	}
				...
				struct device_attribute *attr, char *buf)
{
	...
	cmd = 0x20;
	ret = a96t3x6_i2c_write(data->client, REG_SAR_TOTALCAP, &cmd, 1);
	if (ret < 0)
...
static void a96t3x6_grip_sw_reset(struct a96t3x6_data *data)
{
	...
	SENSOR_INFO("\n");
	ret = a96t3x6_i2c_write(data->client, REG_SW_RESET, &cmd, 1);
	if (ret < 0)
		SENSOR_ERR("fail(%d)\n", ret);
}
		 ...
		 size_t count)
{
	...
	ret = a96t3x6_i2c_write(data->client, REG_SAR_THRESHOLD, &cmd[0], 1);
	if (ret != 0) {
		SENSOR_INFO("failed to write press_threhold data1");
		goto press_threshold_out;
	}
	ret = a96t3x6_i2c_write(data->client, REG_SAR_THRESHOLD + 0x01, &cmd[1], 1);
	if (ret != 0) {
...
static int a96t3x6_fw_mode_check(struct a96t3x6_data *data)
{
	unsigned char buf[1] = {0};
	int ret;

	ret = a96t3x6_i2c_read_data(data->client, buf, 1);
	if (ret < 0) {
		SENSOR_ERR("write fail\n");
		return 0;
	}
	...
	SENSOR_ERR("value is not same, %X, %X\n", FLASH_MODE, buf[0]);
...
static int a96t3x6_fw_update(struct a96t3x6_data *data, u8 cmd)
{
	...
	if (a96t3x6_fw_mode_check(data) != 1) {
		SENSOR_ERR("err, flash mode is not: %d\n", ret);
		return 0;
	}
...
static DEVICE_ATTR(grip_sar_enable, 0664, NULL, grip_sar_enable);
static DEVICE_ATTR(grip_sw_reset, 0664, NULL, grip_sw_reset);
static DEVICE_ATTR(grip_earjack, 0664, NULL, grip_sensing_change);
...
#ifndef CONFIG_SAMSUNG_PRODUCT_SHIP
static DEVICE_ATTR(grip_sar_only_mode, 0664, NULL, grip_mode_change);
static DEVICE_ATTR(grip_sar_press_threshold, 0664,
		NULL, grip_sar_press_threshold_store);
static DEVICE_ATTR(grip_sar_release_threshold, 0664,
...
static DEVICE_ATTR(grip_firm_update, 0664, NULL, grip_fw_update);
static DEVICE_ATTR(grip_firm_update_status, 0664, grip_fw_update_status, NULL);
static DEVICE_ATTR(flip_mode, 0664, NULL, flip_cover_mode_enable);
				  ...
				  const struct i2c_device_id *id)
{
	...
	input_dev->open = a96t3x6_input_open;
	input_dev->close = a96t3x6_input_close;
...
static int a96t3x6_input_open(struct input_dev *dev)
{
	struct a96t3x6_data *data = input_get_drvdata(dev);
