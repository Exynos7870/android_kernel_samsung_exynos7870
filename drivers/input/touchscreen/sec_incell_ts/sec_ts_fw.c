...
int sec_ts_firmware_update_on_probe(struct sec_ts_data *ts, bool force_update)
{
...
#ifdef CONFIG_SEC_JACKPOT2QLTE_PROJECT
	if (!strcmp(ts->plat_data->firmware_name, "tsp_sec/y661_a7y18_rev0.fw") &&
		(ts->plat_data->img_version_of_ic[3]  != ts->plat_data->img_version_of_bin[3]))
		force_update = true;
#endif
	...
	if ((result <= 0) && (!force_update)) {
		...
		ts->tdata->tclm_read(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA);
