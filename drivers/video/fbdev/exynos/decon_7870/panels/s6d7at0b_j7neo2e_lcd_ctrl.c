...
#include "s6d7at0b_j7neo2e_param.h"
...
#include "mdnie.h"
#include "mdnie_lite_table_j7neo2e.h"
#endif
...
static int s6d7at0b_init(struct lcd_info *lcd)
{
	...
	DSI_WRITE(SEQ_S6D7AT0B_F4_01, ARRAY_SIZE(SEQ_S6D7AT0B_F4_01));
	DSI_WRITE(SEQ_S6D7AT0B_F4_02, ARRAY_SIZE(SEQ_S6D7AT0B_F4_02));
	...
	const struct i2c_device_id *id)
{
	...
	dev_err(&lcd->ld->dev, "%s: %s %s done\n", __func__, dev_name(&client->adapter->dev), dev_name(&client->dev));
...
struct mipi_dsim_lcd_driver s6d7at0b_mipi_lcd_driver = {
	...
	.prepare	= dsim_panel_resume_early,
