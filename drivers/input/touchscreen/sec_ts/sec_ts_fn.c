...
#define OPEN_SHORT_PASS		1
#define OPEN_SHORT_FAIL		0

static void send_event_to_user(struct sec_ts_data *ts, int number, int val)
{
	char timestamp[32];
	char feature[32];
	char result[32];
	...
	strncat(test, eol, 1);
	snprintf(result, 32, "%s", val ? "RESULT=PASS" : "RESULT=FAIL");
...
int sec_tclm_data_read(struct i2c_client *client, int address)
{
	...
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_LASTP:
		ret = SEC_TS_NVM_OFFSET_HISTORY_QUEUE_LASTP;
		break;
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_ZERO:
		ret = SEC_TS_NVM_OFFSET_HISTORY_QUEUE_ZERO;
		break;
...
static void run_trx_short_test(void *device_data)
{
	...
	kfree(rBuff);
	send_event_to_user(ts, sec->cmd_param[0], OPEN_SHORT_FAIL);
	return;
...
test_ok:
	...
	kfree(rBuff);
	send_event_to_user(ts, sec->cmd_param[0], OPEN_SHORT_PASS);
	return;
