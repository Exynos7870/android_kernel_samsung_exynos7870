...
#define CS35L40_CAL_VERSION "5.00.4"

#define CS35L40_CAL_CLASS_NAME		"cs35l40"
#define CS35L40_CAL_DIR_NAME		"cs35l40_cal"
...
#define CS35L40_CAL_COMPLETE_DELAY_MS	1500
...
struct cs35l40_cal_t *cs35l40_cal;

struct cs35l40_dsp_region {
	int type;
	unsigned int base;
};
...
static int cs35l40_cal_read_file(char *filename, int *value)
{
	...
	ret = vfs_read(cal_filp, str, sizeof(str), &cal_filp->f_pos);
	if (ret != sizeof(str)) {
...
int cs35l40_cal_apply(void)
{
	...
	ret = cs35l40_cal_read_file(CS35L40_CAL_RDC_SAVE_LOCATION, &rdc);
	if (ret < 0)
		return ret;

	ret = cs35l40_cal_read_file(CS35L40_CAL_TEMP_SAVE_LOCATION, &temp);
	if (ret < 0)
		return ret;
					...
					const char *buf, size_t size)
{
	...
	int ambient, delay = msecs_to_jiffies(CS35L40_CAL_COMPLETE_DELAY_MS);
...
static DEVICE_ATTR(version, S_IRUGO, cs35l40_cal_version_show,
				cs35l40_cal_version_store);
static DEVICE_ATTR(status, S_IRUGO | S_IWUSR | S_IWGRP, cs35l40_cal_status_show,
				cs35l40_cal_status_store);
static DEVICE_ATTR(rdc, S_IRUGO | S_IWUSR | S_IWGRP, cs35l40_cal_rdc_show,
				cs35l40_cal_rdc_store);
static DEVICE_ATTR(temp, S_IRUGO | S_IWUSR | S_IWGRP, cs35l40_cal_temp_show,
				cs35l40_cal_temp_store);
static DEVICE_ATTR(checksum, S_IRUGO | S_IWUSR | S_IWGRP, cs35l40_cal_checksum_show,
				cs35l40_cal_checksum_store);
static DEVICE_ATTR(set_status, S_IRUGO, cs35l40_cal_set_status_show,
				cs35l40_cal_set_status_store);
static DEVICE_ATTR(rdc_stored, S_IRUGO, cs35l40_cal_rdc_stored_show,
				cs35l40_cal_rdc_stored_store);
static DEVICE_ATTR(temp_stored, S_IRUGO, cs35l40_cal_temp_stored_show,
...
static int cs35l40_cal_probe(struct platform_device *pdev)
{
	...
	cs35l40_cal->cal_class = class_create(THIS_MODULE, CS35L40_CAL_CLASS_NAME);
	if (IS_ERR(cs35l40_cal->cal_class)) {
		ret = PTR_ERR(cs35l40_cal->cal_class);
		goto err_alloc;
	}

	cs35l40_cal->dev = device_create(cs35l40_cal->cal_class, NULL, 1, NULL,
...
err_dev:
	class_destroy(cs35l40_cal->cal_class);
err_alloc:
	kfree(cs35l40_cal);
	return ret;
}
...
struct platform_driver cs35l40_cal_driver = {
	.driver = {
		.name = "cs35l40-cal",
		.owner = THIS_MODULE,
	},
