...
void set_tsp_nvm_data_by_size(struct sec_ts_data *ts, u8 length, u8 *data)
{
	int ret;
	u8 buf = length;
	input_info(true, &ts->client->dev, "%s\n", __func__);
	...
	ret = ts->sec_ts_i2c_write(ts, SEC_TS_CMD_NVM_WRITE, data, length + 1);
	if (ret < 0)
		input_err(true, &ts->client->dev, "%s: nvm write failed. ret: %d\n", __func__, ret);
...
int sec_tclm_data_read(struct i2c_client *client, int address)
{
	...
	case SEC_TCLM_NVM_OFFSET_DISASSEMBLE_COUNT:
		return ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_DISASSEMBLE_COUNT];
	...
	case SEC_TCLM_NVM_OFFSET_TUNE_VERSION:
			return (ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_TUNE_VERSION] << 8)
			| ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_TUNE_VERSION + 1];
	...
	case SEC_TCLM_NVM_ALL_DATA:
		ret = get_tsp_nvm_data_by_size(ts, SEC_TCLM_NVM_ALL_SIZE, ts->tdata->nvm_all_data);
		return ret;
	default:
		return 0;
	}
...
void sec_tclm_data_write(struct i2c_client *client, int address, int data)
{
	...
	switch (address) {
	case SEC_TCLM_NVM_OFFSET_FAC_RESULT:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_FAC_RESULT] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_DISASSEMBLE_COUNT:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_DISASSEMBLE_COUNT] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_CAL_COUNT:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_CAL_COUNT] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_TUNE_VERSION:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_TUNE_VERSION] = (u8)(data >> 8);
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_TUNE_VERSION + 1] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_CAL_POSITION:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_CAL_POSITION] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_COUNT:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_HISTORY_QUEUE_COUNT] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_LASTP:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_HISTORY_QUEUE_LASTP] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_ZERO:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_HISTORY_QUEUE_ZERO] = (u8)(0xff & data);
		return;
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_SAVE:
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_HISTORY_QUEUE_ZERO + ts->tdata->cal_pos_hist_lastp * 2] = ts->tdata->cal_position;
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_HISTORY_QUEUE_ZERO + ts->tdata->cal_pos_hist_lastp * 2 + 1] = ts->tdata->cal_count;
		return;
	case SEC_TCLM_NVM_ALL_DATA:
		set_tsp_nvm_data_by_size(ts, SEC_TCLM_NVM_ALL_SIZE, ts->tdata->nvm_all_data);
		return;
	default:
		return;
	}
...
static void set_tsp_test_result(void *device_data)
{
	...
	sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA);
	r_data[0] = sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_OFFSET_FAC_RESULT);
	if (r_data[0] == 0xFF)
		r_data[0] = 0;
	...
	sec_tclm_data_write(ts->tdata->client, SEC_TCLM_NVM_OFFSET_FAC_RESULT, buff[2]);
	sec_tclm_data_write(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA, 0);
	sec_ts_delay(20);
	sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA);
	ts->nv = sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_OFFSET_FAC_RESULT);
...
static void get_tsp_test_result(void *device_data)
{
	...
	memset(buff, 0x00, SEC_CMD_STR_LEN);
	sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA);
	buff[0] = sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_OFFSET_FAC_RESULT);
	if (buff[0] == 0xFF) {
		ts->tdata->nvm_all_data[SEC_TS_NVM_OFFSET_FAC_RESULT] = 0;
		buff[0] = 0;
	}
...
static void increase_disassemble_count(void *device_data)
{
	...
	sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA);
	buff[2] = sec_tclm_data_read(ts->tdata->client, SEC_TS_NVM_OFFSET_DISASSEMBLE_COUNT);
	...
	sec_tclm_data_write(ts->tdata->client, SEC_TCLM_NVM_OFFSET_DISASSEMBLE_COUNT, buff[2]);
	sec_tclm_data_write(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA, 0);
	sec_ts_delay(20);
	sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA);
	buff[0] = sec_tclm_data_read(ts->tdata->client, SEC_TCLM_NVM_OFFSET_DISASSEMBLE_COUNT);
...
static void get_disassemble_count(void *device_data)
{
	...
	if (buff[0] == 0xFF) {
		buff[0] = 0;
		sec_tclm_data_write(ts->tdata->client, SEC_TCLM_NVM_OFFSET_DISASSEMBLE_COUNT, buff[0]);
		sec_tclm_data_write(ts->tdata->client, SEC_TCLM_NVM_ALL_DATA, buff[0]);
	}
